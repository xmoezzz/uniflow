...
std::string createRuleExtData(int Level, const std::string &ruleID) {
  return "[<custom>]" + std::to_string(Level) + ":" + ruleID;
}
...
private:
  ...
  void reportBug(CheckerContext &C, std::unique_ptr<BugType> BT[],
                 ...
                 StringRef Desc, StringRef CheckerData = "") const;
  ...
  if (const LockState *LState = state->get<LockMap>(lockR)) {
    if (LState->isLocked()) {
      reportBug(C, BT_doublelock, MtxExpr, CheckKind,
                ...
                createRuleExtData(1, "PthreadLockChecker"));
      return;
...
void PthreadLockChecker::reportBug(CheckerContext &C,
  ...
  auto Report =
      std::make_unique<PathSensitiveBugReport>(*BT[CheckKind], CheckerData, Desc, N);
