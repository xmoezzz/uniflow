...
namespace {
    ...
    class UnsignedIntegerOverflowChecker : public Checker< check::ASTCodeBody, check::PostStmt<BinaryOperator> > {
...
void UnsignedIntegerOverflowChecker::checkASTCodeBody(const Decl* D, AnalysisManager& Mgr,
...
void UnsignedIntegerOverflowChecker::checkPostStmt(const BinaryOperator* BO, CheckerContext& C) const {
...
bool UnsignedIntegerOverflowChecker::checkOverflow(const BinaryOperator* BO, CheckerContext& C) const {
    if (!BO->isAdditiveOp())
        return false;
...
void UnsignedIntegerOverflowChecker::reportBug(const Decl* FD, const SourceLocation& Loc, BugReporter& BR) const {
	if (Loc.isMacroID())
		return;
	
    if (!BT)
        BT.reset(new BuiltinBug(this, "UnsignedIntegerOverflowChecker"));

    // Report the issue
	auto ls = anzulocalization::LocaleSetting::getInstance();
	uint64_t lang = GetOutputLocaleSetting(BR.getAnalyzerOptions(), ls->getSupportedLangMask());
	std::string Msg = ls->parseMsgs(anzulocalization::UnsignedIntegerOverflowChecker, lang);        
    PathDiagnosticLocation DLoc(Loc, BR.getSourceManager());
    auto Report = std::make_unique<BasicBugReport>(
        *BT, Msg, createRuleExtData(1, "UnsignedIntegerOverflowChecker"), DLoc);
...
#if RELEASE_BUNDLE
void ento::registerUnsignedIntegerOverflowChecker(CheckerManager& Mgr) {
    Mgr.registerChecker<UnsignedIntegerOverflowChecker>();
}

bool ento::shouldRegisterUnsignedIntegerOverflowChecker(const CheckerManager& mgr) {
...
void clang_registerCheckers(CheckerRegistry & registry) {
    registry.addChecker<UnsignedIntegerOverflowChecker>("anzu1.UnsignedIntegerOverflowChecker", "Unsigned integer wraparound", "");
}
