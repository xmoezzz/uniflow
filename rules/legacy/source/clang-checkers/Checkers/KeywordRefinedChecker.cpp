...
namespace {
	...
	class KeywordRefinedChecker : public Checker<check::EndOfTranslationUnit> {
		...
		void reportBug(const SourceLocation& Loc, BugReporter& BR) const {
			...
			if (!BT) {
				BT.reset(new BuiltinBug(
					this, "KeywordRefinedChecker"));
			}

			// Report the issue
			auto ls = anzulocalization::LocaleSetting::getInstance();
			uint64_t lang = GetOutputLocaleSetting(BR.getAnalyzerOptions(), ls->getSupportedLangMask());
			std::string Msg = ls->parseMsgs(anzulocalization::KeywordRefinedChecker, lang);
			PathDiagnosticLocation DLoc(Loc, BR.getSourceManager());
			auto Report = std::make_unique<BasicBugReport>(
				*BT, Msg, createRuleExtData(1, "KeywordRefinedChecker"), DLoc);
...
#if RELEASE_BUNDLE
void ento::registerKeywordRefinedChecker(CheckerManager& Mgr) {
	Mgr.registerChecker<KeywordRefinedChecker>();
}

bool ento::shouldRegisterKeywordRefinedChecker(const CheckerManager& mgr) {
...
void clang_registerCheckers(CheckerRegistry & registry) {
	registry.addChecker<KeywordRefinedChecker>("anzu.KeywordRefinedChecker", "", "");
}
