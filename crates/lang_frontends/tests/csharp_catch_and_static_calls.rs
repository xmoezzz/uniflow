...
fn csharp_catches_keep_types_scopes_and_static_calls_are_not_declarations() {
    ...
    let Stmt::Try { catches, .. } = &method.body.stmts[0] else { panic!("try"); };
    assert_eq!(catches.len(), 2);
    let type_name = |id| program.types.iter().find(|ty| ty.id == id).unwrap().name.as_str();
    ...
    for catch in catches {
        ...
        let Stmt::Expr { expr: Expr::Call(call), .. } = &catch.body.stmts[0] else { panic!("catch call"); };
        let Expr::VarRef { symbol, .. } = &call.args[0] else { panic!("exception reference"); };
        assert_eq!(Some(*symbol), catch.symbol);
    }
    let Stmt::Expr { expr: Expr::Call(call), .. } = &method.body.stmts[1] else { panic!("outer call"); };
    assert!(matches!(&call.args[0], Expr::VarRef { symbol, .. } if *symbol == method.params[0].symbol));
    let Stmt::Expr { expr: Expr::Call(call), .. } = &method.body.stmts[2] else { panic!("static File.Create must remain a call"); };
    assert!(matches!(&call.target, CallTarget::Named(name) if name == "Create"));
    assert!(call.qualifier_is_explicit);
    let Stmt::Let { ty: Some(ty), init: Some(Expr::New { type_name: allocated, .. }), .. }
        = &method.body.stmts[3] else { panic!("qualified declaration"); };
