...
fn indexes_nested_if_branches_without_confusing_end_if() {
    ...
    let outer = syntax.ifs.iter().find(|item| item.branches.len() == 3).unwrap();
    assert_eq!(syntax.normalized(outer.branches[0].condition.clone().unwrap()), "a");
    assert_eq!(syntax.normalized(outer.branches[1].condition.clone().unwrap()), "c");
