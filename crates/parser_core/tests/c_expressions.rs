...
fn count(source: &str, kind: K) -> usize {
    CExpressionIndex::parse(source).facts.iter().filter(|fact| fact.kind == kind).count()
}
...
fn c_expression_contexts_track_nested_calls_for_and_sizeof() {
    ...
    let commas = index.facts.iter().filter(|fact| fact.kind == K::Comma).collect::<Vec<_>>();
    assert!(commas[0].inside_for_header);
    assert_eq!(commas[1].enclosing_calls, ["use"]);
    let dangerous = index.tokens.iter().find(|token| token.text == "KEY_ALL_ACCESS").unwrap();
    assert_eq!(index.nearest_call_name(dangerous.start as usize).as_deref(), Some("nested"));
    assert_eq!(index.facts.iter().filter(|fact| fact.inside_sizeof && matches!(fact.kind, K::Assignment | K::Update | K::Call)).count(), 3);
}
...
fn c_expression_groups_are_smallest_balanced_owners() {
    ...
    let binary = index.facts.iter().find(|fact| fact.kind == K::Binary).unwrap();
    let (open, close) = index.smallest_group(binary.offset).unwrap();
    assert_eq!(&source[index.tokens[open].start as usize..index.tokens[close].end as usize], "(a + (b ? c : d))");
    for malformed in ["(", "f(a", "sizeof (x =", "a ? b", "RegOpenKeyEx(KEY_ALL_ACCESS"] {
