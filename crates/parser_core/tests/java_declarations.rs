...
"#;
    ...
    let outer = declarations
        .iter()
        .position(|d| d.name == "Outer" && d.kind == D::Class)
        .unwrap();
    assert_eq!(
        declarations[outer].interfaces,
        ["java.io.Serializable", "Cloneable"]
    );
    assert_eq!(declarations[outer].superclass, "Thread");
    let field = declarations
        .iter()
        .find(|d| d.kind == D::Field && d.names.contains(&"first".into()))
        .unwrap();
    ...
    let members = declarations
        .iter()
        .filter(|d| d.owner == Some(outer) && d.is_member)
        .collect::<Vec<_>>();
    assert_eq!(members.len(), 7, "{declarations:#?}");
    assert!(!members
        .iter()
        .any(|d| d.name == "hashCode" || d.name == "Local"));
    assert_eq!(
        declarations
            .iter()
            .filter(|d| d.kind == D::AnonymousClass)
            .count(),
        1
    );
    assert_eq!(
        declarations.iter().filter(|d| d.name == "hashCode").count(),
        3
    );
    assert!(members
        .iter()
        .any(|d| d.name == "pending" && d.kind == D::Method));
}
