...
impl JavaSyntax {
    ...
    pub fn statement_ranges(source: &str) -> Vec<Range<usize>> {
        ...
        index
            .roots
            .iter()
            .map(|&id| index.nodes[id].range.clone())
            .collect()
    }
    ...
    fn lex(source: &str) -> Self {
        ...
        Self {
            ...
            tokens,
            mates,
            csharp_attributes: false,
            c_family_types: false,
        }
    }
    ...
    fn is(&self, i: usize, text: &str) -> bool {
        self.tokens
            .get(i)
            .is_some_and(|t| t.kind != TokKind::StringLit && t.text == text)
    }
    ...
    fn sequence(&mut self, mut pos: usize, end: usize, depth: usize) -> Vec<usize> {
        ...
        while pos < end {
            ...
            if let Some(&previous) = ids.last() {
                ...
                self.nodes[previous].followed_by_block =
                    self.nodes[id].kind == JavaSyntaxKind::Block;
            }
            ids.push(id);
            pos = next.max(pos + 1);
        }
    ...
    fn declarations(&mut self, mut start: usize, end: usize, class_name: &str, depth: usize) {
        ...
        while start < end {
            ...
            while cursor < end {
                ...
                if self.is(cursor, "{") {
                    let Some(close) = self.mates[cursor] else {
                        break;
                    };
                    ...
                    {
                        let constructor =
                            method_name.as_deref() == Some(class_name) || method_name.is_none();
    ...
    fn statement(&mut self, start: usize, end: usize, depth: usize) -> (usize, usize) {
        ...
        if depth > 256 {
            return (end, self.add(K::Other, start, end));
        }
        if self.is(start, ";") {
            return (start + 1, self.add(K::Empty, start, start + 1));
        }
        if self.is(start, "{") {
        ...
        let kind = match keyword {
            ...
            "while" => Some(K::While),
            "for" => Some(K::For),
            "synchronized" => Some(K::Synchronized),
            "switch" => Some(K::Switch),
            _ => None,
        };
            ...
            if self.is(start + 1, "(") {
                if let Some(close) = self.mates[start + 1].filter(|&c| c + 1 < end) {
                    ...
                    let (mut next, _) = if kind == K::Switch && self.is(body_start, "{") {
                        ...
                        self.statement(body_start, end, depth + 1)
                    };
        ...
        if self.is(start, "try") {
            ...
            if self.is(body, "(") {
                body = self.mates[body].map_or(body, |c| c + 1);
            }
            if self.is(body, "{") {
                ...
                while next < end && (self.is(next, "catch") || self.is(next, "finally")) {
                    ...
                    if self.is(block, "(") {
                        block = self.mates[block].map_or(block, |c| c + 1);
                    }
                    if !self.is(block, "{") {
                        break;
                    }
        ...
        while next < end {
            ...
            if let Some(close) = self.mates[next].filter(|&c| c > next) {
                next = close + 1;
            } else {
                next += 1;
            }
            ...
            if self.is(i, "{") {
                if let Some(close) = self.mates[i] {
                    if i > start && self.is(i - 1, "->") {
                        ...
                        self.expression_bodies(i + 1, close, depth + 1);
                    }
                    i = close + 1;
                    continue;
                }
            ...
            if self.is(start, "case") || self.is(start, "default") {
                ...
                loop {
                    ...
                    while start < end && !self.is(start, ":") && !self.is(start, "->") {
                        start = self.mates[start]
                            .filter(|&c| c > start)
                            .map_or(start + 1, |c| c + 1);
                    }
                    arrow_label |= self.is(start, "->");
                    start += usize::from(start < end);
                    if start >= end || (!self.is(start, "case") && !self.is(start, "default")) {
                        break;
                    }
                }
                ...
                if body_start < start {
                    self.sequence(body_start, start, depth + 1);
                }
            } else {
    ...
    pub fn tokens_in(&self, range: Range<usize>) -> impl Iterator<Item = &Token> {
        ...
        self.tokens[start..]
            .iter()
            .take_while(move |t| (t.start as usize) < range.end)
    }
