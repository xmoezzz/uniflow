...
impl CExpressionIndex {
    pub fn parse(source: &str) -> Self {
        ...
        for at in 0..tokens.len() {
            ...
            if tokens[at].kind != TokKind::Symbol {
                continue;
            }
            match tokens[at].text.as_str() {
                "(" | "[" | "{" => stack.push(at),
                ")" | "]" | "}" => {
                    let wanted = match tokens[at].text.as_str() {
                        ")" => "(",
                        "]" => "[",
                        _ => "{",
                    };
                    if stack
                        .last()
                        .is_some_and(|open| tokens[*open].text == wanted)
                    {
        ...
        for at in 0..result.tokens.len().saturating_sub(1) {
            ...
            let kind = if matches!(
                ...
                Some(CExpressionFactKind::Update)
            } else if text == "?" {
                Some(CExpressionFactKind::Conditional)
            } else if text == "," {
                Some(CExpressionFactKind::Comma)
            } else if is_binary(text) {
                Some(CExpressionFactKind::Binary)
            } else if text == "(" && result.call_name(at).is_some() {
                Some(CExpressionFactKind::Call)
            } else {
                None
            };
            if let Some(kind) = kind {
                ...
                let offset = if kind == CExpressionFactKind::Call {
                    ...
                    result.tokens[at].start as usize
                };
                result.facts.push(CExpressionFact {
                    kind,
                    offset,
                    end: result.tokens[at].end as usize,
                    enclosing_calls,
                    inside_for_header,
                    inside_sizeof,
                });
    ...
    fn context(&self, at: usize) -> (bool, bool, Vec<String>) {
        ...
        while let Some(group) = open {
            if self.tokens[group].text == "(" {
                if self
                    .tokens
                    .get(group.wrapping_sub(1))
                    .is_some_and(|token| token.text == "for")
                {
                    inside_for = true;
                }
                if self
                    .tokens
                    .get(group.wrapping_sub(1))
                    .is_some_and(|token| token.text == "sizeof")
                {
                    inside_sizeof = true;
                }
                if let Some(name) = self.call_name(group) {
                    calls.push(name);
                }
            }
        ...
        if !inside_sizeof {
            inside_sizeof = (0..at)
                ...
                .any(|index| {
                    self.tokens[index].text == "sizeof"
                        && !self.is_group_closed_before(index + 1, at)
                });
        }
    ...
    fn call_name(&self, open: usize) -> Option<String> {
        if !self.tokens.get(open).is_some_and(|token| token.text == "(") || open == 0 {
            return None;
        }
    ...
    pub fn token_at(&self, offset: usize) -> Option<&Token> {
        self.tokens
            .iter()
            .find(|token| token.start as usize == offset)
    }
    ...
    pub fn smallest_group(&self, offset: usize) -> Option<(usize, usize)> {
        self.tokens
            .iter()
            .enumerate()
            .filter(|(_, token)| token.start as usize <= offset)
            .filter_map(|(open, _)| self.mates[open].map(|close| (open, close)))
            .filter(|(open, close)| {
                self.tokens[*open].start as usize <= offset
                    && self.tokens[*close].end as usize > offset
            })
    ...
    pub fn inside_control_header(&self, offset: usize, keyword: &str) -> bool {
        ...
        let mut open = self.parents[at];
        while let Some(group) = open {
            if self.tokens[group].text == "(" && group > 0 && self.tokens[group - 1].text == keyword
            {
                return true;
            }
            open = self.parents[group];
    ...
    pub fn inside_control_condition(&self, offset: usize) -> bool {
        let Some(at) = self
            .tokens
            .iter()
            .position(|token| token.start as usize == offset)
        ...
        let mut open = self.parents[at];
        while let Some(group) = open {
            if self.tokens[group].text == "(" && group > 0 {
                match self.tokens[group - 1].text.as_str() {
                    "if" | "while" | "switch" => return true,
                    "for" => {
                        let Some(close) = self.mates[group] else {
                            return false;
                        };
                        let separators = (group + 1..close)
                            .filter(|index| {
                                self.parents[*index] == Some(group)
                                    && self.tokens[*index].text == ";"
                            })
                            .collect::<Vec<_>>();
                        return separators.len() == 2
                            && separators[0] < at
                            && at < separators[1];
                    }
                    _ => {}
    ...
    pub fn statement_range(&self, offset: usize) -> Option<Range<usize>> {
        let at = self
            .tokens
            .iter()
            .position(|token| token.start as usize <= offset && offset < token.end as usize)?;
        ...
        let start = (0..at)
            .rev()
            .find(|index| {
                direct(*index) && matches!(self.tokens[*index].text.as_str(), ";" | "{" | "}")
            })
            .map_or(0, |index| self.tokens[index].end as usize);
        let end = (at..self.tokens.len())
            .find(|index| direct(*index) && self.tokens[*index].text == ";")
