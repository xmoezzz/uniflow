...
impl JavaDeclaration {
    fn new(kind: JavaDeclarationKind, range: Range<usize>, owner: Option<usize>) -> Self {
        Self {
            ...
            modifiers: Vec::new(),
            annotations: Vec::new(),
            marker_annotations: Vec::new(),
            annotation_arguments: Vec::new(),
...
impl JavaSyntax {
    ...
    fn declaration_prefix(
        ...
        while at < end {
            if self.csharp_attributes && self.is(at, "[") {
                let Some(close) = self.mates[at] else {
                    break;
                };
                let mut part = at + 1;
                ...
                if at < end {
                    name.push_str(&self.tokens[at].text);
                    at += 1;
                }
                while at + 1 < end && self.is(at, ".") {
                    name.push('.');
                    name.push_str(&self.tokens[at + 1].text);
                    at += 2;
                }
                if !self.is(at, "(") {
                ...
                if self.is(at, "(") {
                    if let Some(close) = self.mates[at] {
                        arguments.push((
                            annotations.last().unwrap().clone(),
                            self.token_text(at + 1, close),
                        ));
                        at = close + 1;
                    } else {
                        at += 1;
                ...
                modifiers.push(self.tokens[at].text.clone());
                at += 1;
            } else if self.is(at, "non") && self.is(at + 1, "-") && self.is(at + 2, "sealed") {
                modifiers.push("non-sealed".into());
                at += 3;
            } else {
                break;
            }
    ...
    fn declaration_region(
        ...
        while start < end {
            ...
            let (mut head, modifiers, annotations, markers, arguments) =
                self.declaration_prefix(start, end);
            if self.is(head, "@") && self.is(head + 1, "interface") {
                head += 1;
            }
            let kind = match self.tokens.get(head).map(|t| t.text.as_str()) {
                Some("class") => Some(D::Class),
                Some("interface") => Some(D::Interface),
                Some("enum") => Some(D::Enum),
                Some("record") => Some(D::Record),
                _ => None,
            };
            if let Some(kind) = kind {
                let mut open = head + 2;
                while open < end && !self.is(open, "{") && !self.is(open, ";") {
                    open = self.mates[open]
                        .filter(|&c| c > open)
                        .map_or(open + 1, |c| c + 1);
                }
                if let Some(close) = self.mates.get(open).copied().flatten().filter(|&c| c < end) {
                    let mut declaration =
                        JavaDeclaration::new(kind, self.range(start, close + 1), owner);
            ...
            if self.is(head, "{") {
                if let Some(close) = self.mates[head] {
                    let mut declaration =
                        JavaDeclaration::new(D::Initializer, self.range(start, close + 1), owner);
                    declaration.is_member = is_member;
                    declaration.modifiers = modifiers;
                    self.declarations.push(declaration);
            ...
            if let Some(params) = method {
                if let Some(close_params) = self.mates[params] {
                    ...
                    while body < end && !self.is(body, "{") && !self.is(body, ";") {
                        body += 1;
                    }
                    let next = if self.is(body, "{") {
                        self.mates[body].map_or(body + 1, |c| c + 1)
                    } else {
                        (body + 1).min(end)
                    };
                    let name = self.tokens[params - 1].text.clone();
                    let constructor = owner.is_some_and(|id| self.declarations[id].name == name);
                    let mut declaration = JavaDeclaration::new(
                        if constructor {
                            D::Constructor
                        } else {
                            D::Method
                    ...
                    if self.is(body, "{") {
                        declaration.body = Some(self.range(body, next));
                    }
            ...
            if owner.is_some() && cursor < end && self.is(cursor, ";") && head < cursor {
                let mut field =
                    JavaDeclaration::new(D::Field, self.range(start, cursor + 1), owner);
    ...
    fn type_bases(&self, mut at: usize, end: usize, declaration: &mut JavaDeclaration) {
        ...
        while at <= end {
            ...
            if angle == 0 && matches!(word, "extends" | "implements" | "permits" | ",") {
                if !name.is_empty() {
                    if mode == "extends" {
                        declaration.superclass = std::mem::take(&mut name);
                    } else if mode == "implements" {
                        declaration.interfaces.push(std::mem::take(&mut name));
                    } else {
                        name.clear();
                    }
                }
                ...
                angle = (angle - word.len() as i32).max(0);
            } else if angle == 0
                && !mode.is_empty()
                && (word == "."
                    || self
            ...
            if at < end && self.is(at, "(") {
                at = self.mates[at].unwrap_or(at);
            }
            at += 1;
        }
    }
    ...
    fn field_declarators(&self, start: usize, end: usize, field: &mut JavaDeclaration) {
        ...
        while at <= end {
            ...
            if !assigned && matches!(word, ">" | ">>" | ">>>") {
                angle = (angle - word.len() as i32).max(0);
            }
            if angle == 0 && word == "=" && !assigned {
                name_end = Some(at);
                assigned = true;
            }
            if angle == 0 && word == "," {
                let until = name_end.unwrap_or(at);
                if let Some(name) = (segment..until)
                    .rev()
                    .find(|&i| self.tokens[i].kind == TokKind::Ident)
                {
    ...
    fn embedded_declarations(
        ...
        while at < end {
            ...
            if ["class", "interface", "enum", "record"]
                .iter()
                .any(|word| self.is(head, word))
            ...
            if self.is(at, "{") && self.anonymous_creation(at).is_some() {
                if let Some(close) = self.mates[at] {
                    ...
                    let id = self.declarations.len();
                    let mut declaration = JavaDeclaration::new(
                        JavaDeclarationKind::AnonymousClass,
                        self.range(creation, close + 1),
                        owner,
                    );
                    declaration.parameters = Some(self.range(params + 1, close_params));
                    self.declarations.push(declaration);
    ...
    fn anonymous_creation(&self, open: usize) -> Option<(usize, usize, usize)> {
        ...
        for at in (0..params).rev() {
            if self.is(at, "new") {
                return Some((at, params, close_params));
            }
            if matches!(
                self.tokens[at].text.as_str(),
                ";" | "{"
