...
fn findings(source: &str) -> Vec<String> {
    ...
    builtin_security_pack().expect("pack").scan_hir(
        &program,
        &HashMap::from([("Input.cs".to_string(), source.to_string())]),
    ).into_iter().map(|finding| finding.rule_id).collect()
}
...
fn check_rule(rule: &str, source: &str, expected: usize) {
    ...
    let actual = pack.scan_hir(&program,
...
fn migrated_csharp_sql_adapter_requires_declared_source_provenance() {
    for rule in ["LEGACY-CS-AST-SCS0002", "LEGACY-CS-AST-security_risky_sql_query"] {
        check_rule(rule, "class A { void F(string p) { var x = new SqlDataAdapter(\"select \" + p, conn); } }", 1);
        ...
        check_rule(rule, "class A { void F() { string q = helper(); var x = new SqlDataAdapter(q, conn); } }", 0);
        check_rule(rule, "class A { void F(int p) { var x = new SqlDataAdapter(p.ToString(), conn); } }", 0);
        check_rule(rule, "class A { void F(string p) { var x = new SqlDataAdapter(\"fixed\", conn); } }", 0);
    }
}
...
fn migrated_csharp_typed_query_rules_match_nested_declaration_references() {
    for (rule, ty, call) in [
        ...
        ("LEGACY-CS-AST-input_validation_and_representation_linq", "DataContext", "ExecuteCommand"),
        ("LEGACY-CS-AST-input_validation_and_representation_nhibernate", "ISession", "CreateQuery"),
        ("LEGACY-CS-AST-input_validation_and_representation_xquery_injection", "XQueryCompiler", "Compile"),
    ] {
        check_rule(rule, &format!("class A {{ void F({ty} api, string p) {{ api.{call}(\"prefix\" + p); }} }}"), 1);
        check_rule(rule, &format!("class A {{ void F({ty} api) {{ string p = \"constant\"; api.{call}(p); }} }}"), 1);
        check_rule(rule, &format!("class A {{ void F({ty} api) {{ api.{call}(\"constant\"); }} }}"), 0);
        check_rule(rule, &format!("class A {{ void F({ty} api, int p) {{ api.{call}(p.ToString()); }} }}"), 0);
        check_rule(rule, &format!("class A {{ void F(Unrelated api, string p) {{ api.{call}(p); }} }}"), 0);
    }
}
...
fn migrated_csharp_log_forging_distinguishes_parameters_and_initializers() {
    ...
    check_rule(rule, "class A { void F(string p, Logger log) { log.Warn(\"prefix\" + p); } }", 1);
    check_rule(rule, "class A { void F(Logger log) { string p = Request.Form[\"p\"]; log.Warn(p); } }", 1);
    check_rule(rule, "class A { void F(Logger log) { string p = \"fixed\"; log.Warn(p); } }", 0);
    // Original log rule accepts member access, not direct Request[...] access.
    check_rule(rule, "class A { void F(Logger log) { string p = Request[\"p\"]; log.Warn(p); } }", 0);
}

#[test]
fn migrated_csharp_file_paths_use_each_apis_argument_position() {
    let rule = "LEGACY-CS-AST-SCS0018";
    for statement in ["File.Create(p);", "Request.WriteFile(p);", "Request.MapPath(p);",
        "Request.TransmitFile(p);", "provider.GetFileInfo(p);",
        "client.DownloadFile(\"url\", p);", "client.DownloadFileAsync(\"url\", p);",
        "client.DownloadFileTaskAsync(\"url\", p);"] {
        check_rule(rule, &format!("class A {{ void F(string p) {{ {statement} }} }}"), 1);
    }
    for statement in ["Unrelated.Create(p);", "Response.MapPath(p);", "client.DownloadFile(p, \"fixed\");",
        "client.DownloadFileAsync(p, \"fixed\");", "client.DownloadFile(p);", "GetFileInfo(p);"] {
        check_rule(rule, &format!("class A {{ void F(string p) {{ {statement} }} }}"), 0);
    }
    check_rule(rule, "class A { void F() { string p = Response.Value; File.Create(p); } }", 1);
    check_rule(rule, "class A { void F() { string p = Request.Value; File.Create(p); } }", 0);
    check_rule(rule, "class A { void F() { string p = \"fixed\"; File.Create(p); } }", 0);
}
...
fn migrated_csharp_exception_disclosure_requires_catch_receiver_and_exception() {
    for (rule, receiver) in [
        ...
        ("LEGACY-CS-AST-encapsulation_system_information_leak", "Console"),
    ] {
        check_rule(rule, &format!("class A {{ void F() {{ try {{ Work(); }} catch(Exception ex) {{ {receiver}.Write(\"error\", ex.Message); }} }} }}"), 1);
        check_rule(rule, &format!("class A {{ void F(Exception ex) {{ {receiver}.Write(ex); }} }}"), 0);
        check_rule(rule, &format!("class A {{ void F() {{ try {{ Work(); }} catch(Exception ex) {{ {receiver}.Write(\"fixed\"); }} }} }}"), 0);
        check_rule(rule, "class A { void F() { try { Work(); } catch(Exception ex) { Other.Write(ex); } } }", 0);
...
fn migrated_csharp_websocket_options_preserve_same_symbol_exclusion() {
    ...
    check_rule(rule, "class A { void F(object options) { context.AcceptWebSocketRequest(handler, options); } }", 0);
...
fn migrated_csharp_redirect_and_cookie_preserve_legacy_if_exclusions() {
    for (rule, statement) in [
        ...
        ("LEGACY-CS-AST-input_validation_and_representation_cookies", "var c = new Cookie(\"n\", p);")
    ] {
        check_rule(rule, &format!("class A {{ void F(string p) {{ {statement} }} }}"), 1);
        check_rule(rule, &format!("class A {{ void F(string p) {{ if(IsLocal(p)) {{ {statement} }} }} }}"), 0);
        check_rule(rule, &format!("class A {{ void F(string p) {{ if(IsLocal(p)) {{ Work(); }} {statement} }} }}"), 0);
        check_rule(rule, &format!("class A {{ void F(string p) {{ {statement} if(IsLocal(p)) {{ Work(); }} }} }}"), 1);
    ...
    check_rule("LEGACY-CS-AST-SCS0027", "class A { void F(string p) { Other.Redirect(p); } }", 0);
}

#[test]
fn migrated_csharp_xss_header_uses_structural_receiver_and_argument_tokens() {
    let rule = "LEGACY-CS-AST-xss_protection";
    for statement in ["Response.AddHeader(\"X-XSS-Protection\", 0);",
        ...
        check_rule(rule, &format!("class A {{ void F(HttpResponse response) {{ {statement} }} }}"), 1);
    }
    for statement in ["Other.AddHeader(\"X-XSS-Protection\", 0);",
        ...
        "/* Response.Headers.Remove(\"X-XSS-Protection\"); */"] {
        check_rule(rule, &format!("class A {{ void F(HttpResponse response) {{ {statement} }} }}"), 0);
    }
}
...
fn migrated_csharp_upload_encoding_and_template_rules_have_negative_cases() {
    for (rule, parameters, positive, negative) in [
        ("LEGACY-CS-AST-input_validation_and_representation_file_upload_1", "HtmlInputFile input, object other", "var f = input.PostedFile;", "var f = other.PostedFile;"),
        ("LEGACY-CS-AST-input_validation_and_representation_file_upload_2", "FileUpload input, object other", "input.SaveAs(path);", "other.SaveAs(path);"),
        ("LEGACY-CS-AST-input_validation_and_representation_poor_validation", "string p", "HttpUtility.HtmlEncode(p);", "Unrelated.HtmlEncode(p);"),
        ("LEGACY-CS-AST-input_validation_and_representation_razor", "string p", "Razor.Parse(\"prefix\" + p, model);", "Other.Parse(p, model);"),
        ("LEGACY-CS-AST-input_validation_and_representation_castle_activerecord", "string p", "var q = new SimpleQuery<Model>(\"prefix\" + p);", "var q = new SimpleQuery<Model>(\"fixed\");"),
        ("LEGACY-CS-AST-input_validation_and_representation_subsonic", "string p", "var q = new InlineQuery(\"prefix\" + p);", "var q = new InlineQuery(\"fixed\");"),
    ] {
        check_rule(rule, &format!("class A {{ void F({parameters}) {{ {positive} }} }}"), 1);
        check_rule(rule, &format!("class A {{ void F({parameters}) {{ {negative} }} }}"), 0);
        check_rule(rule, &format!("class A {{ void F({parameters}) {{ /* {positive} */ }} }}"), 0);
    }
}
...
fn migrated_csharp_antiforgery_attributes_are_owned_by_the_method() {
    ...
    for (source, expected) in [
        ...
        ("class A { [ValidateAntiForgeryToken] [HttpPost] void Safe() {} }", 0),
        ("class A { [HttpPost] [ValidateAntiForgeryToken] void Safe() {} }", 0),
        ("class A { [HttpPost, ValidateAntiForgeryToken] void Safe() {} }", 0),
        ("class A { [HttpPost] void Bad() {} [ValidateAntiForgeryToken] void Other() {} }", 1),
        ...
        ("class A { [HttpPost()] void Ordinary() {} [Mvc.HttpPost] void Other() {} }", 0),
        ("class A { [HttpPost] [ValidateAntiForgeryToken()] void Bad() {} }", 1),
        ("class A { [Note(values = new[] {1, 2})] [HttpPost] public async Task Bad() {} }", 1),
        ("class A { class B { [HttpPost] void Bad() {} } [HttpGet] void Good() {} }", 1),
    ] {
        ...
        let integrated = pack.scan_hir(&program,
...
"#;
    ...
    for rule in [
        ...
        assert!(actual.iter().any(|candidate| candidate == rule), "missing {rule}: {actual:#?}");
    }
    assert_eq!(actual.iter().filter(|id| id.as_str() == "LEGACY-CS-AST-SCS0016").count(), 1, "{actual:#?}");
    assert_eq!(actual.iter().filter(|id| id.as_str() == "LEGACY-CS-AST-SCS0002").count(), 1, "{actual:#?}");
    assert_eq!(actual.iter().filter(|id| id.as_str() == "LEGACY-CS-AST-SCS0027").count(), 1, "{actual:#?}");
}
