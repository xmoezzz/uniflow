...
fn check(rule: &str, source: &str, expected: usize) {
    ...
    let hir = JavaParser::default().parse_file("Usage.java", source).unwrap();
...
fn migrated_java_usage_rules() {
    ...
    check(field, "class A { private int used; private int dead; int read(){ return used; } }", 1);
    check(field, "@Generated class A { int dead; } class B { @Generated int ignored; int live; void f(){ live++; } }", 0);
    check(field, "class A { int value; void f(int value){ consume(value); } }", 1);

    let method = "LEGACY-JAVA-AST-unused-method";
    check(method, "class A { private void used(){} private void dead(){} void run(){ used(); } }", 1);
    check(method, "class A { @Override private void hook(){} private void recursive(){ recursive(); } }", 1);
    check(method, "class A { void api(){} protected void helper(){} }", 0);

    let variable = "LEGACY-JAVA-AST-unused-variable";
    check(variable, "class A { int f(int p){ int x=0; x = p; return 1; } }", 1);
    check(variable, "class A { int f(int p){ int x=0; x = p; return x; } }", 0);
    ...
    check(inner, "class Outer { int state; public class View { int read(){ return state; } } }", 1);
    check(inner, "class Outer { int state; private class View { int read(){ return state; } } }", 0);
    check(inner, "class Outer { int state; public class View { int read(int state){ return state; } } }", 0);

    let immutable = "LEGACY-JAVA-AST-immutable-field";
    check(immutable, "@Immutable class A { int count; List values; void f(){ count = 1; values.add(1); } }", 2);
    check(immutable, "@Immutable class A { int count; void f(int count){ count = 1; } }", 0);
    check(immutable, "@Immutable(reason=\"documented\") class A { int count; void f(){ count = 1; } }", 0);
    check(immutable, "class A { int count; void f(){ count = 1; } }", 0);
}
