...
fn check(rule: &str, source: &str, expected: usize) {
    ...
    let hir = JavaParser::default()
        .parse_file("MemberCalls.java", source)
        .unwrap();
    let integrated = pack.scan_hir(
        &hir,
        &HashMap::from([("MemberCalls.java".into(), source.into())]),
    );
    assert_eq!(
        integrated.len(),
        expected,
...
fn migrated_java_member_call_rules() {
    ...
    check(
        clone_override,
        "class A { String clone(){ hook(); return \"x\"; } void hook(){} }",
        0,
    );
    ...
    check(
        ctor_override,
        "class A { A(){ hook(); this.safe(); target.hook(); } void hook(){} final void safe(){} }",
        1,
    );
    check(
        ctor_override,
        "class A { A(){ safe(); } final void safe(){} }",
        0,
    );
    check(
        ctor_override,
        "class A { void create(){ hook(); } void hook(){} }",
        0,
    );
    ...
    check(
        security,
        "class A { public void verify(SecurityManager sm){ sm.checkPermission(null); } }",
        1,
    );
    check(
        security,
        "class A { protected void verify(){ SecurityManager sm=get(); sm.checkRead(\"x\"); } }",
        1,
    );
    ...
    check(
        security,
        "class A { public void verify(Object sm){ sm.checkPermission(null); } }",
        0,
    );
    ...
    check(
        read_object,
        "class A { void readObject(ObjectInputStream in){ validate(); } void validate(){} }",
        1,
    );
    check(read_object, "class A { void readObject(java.io.ObjectInputStream in){ first(); } final void first(){ second(); } void second(){} }", 1);
    check(
        read_object,
        "class A { void readObject(ObjectInputStream in){ validate(); } final void validate(){} }",
        0,
    );
    check(
        read_object,
        "final class A { void readObject(ObjectInputStream in){ validate(); } void validate(){} }",
        0,
    );
    check(
        read_object,
        "class A { void readObject(String in){ validate(); } void validate(){} }",
        0,
    );
}
