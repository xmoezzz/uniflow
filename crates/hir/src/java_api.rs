...
static RETURNS: LazyLock<BTreeMap<(&'static str, &'static str, usize), &'static str>> = LazyLock::new(|| {
    let mut result = BTreeMap::new();
    for line in SIGNATURES.lines().filter(|line| !line.is_empty() && !line.starts_with('#')) {
        ...
        for arity in fields[2].split(',') {
            ...
            assert!(result.insert((fields[0], fields[1], arity), fields[3]).is_none(), "duplicate Java signature: {line}");
        }
    }
    result
});
...
mod tests {
    ...
    fn embedded_java_return_signatures_are_exact_and_arity_gated() {
        ...
        for (&(owner, method, arity), &result) in RETURNS.iter() {
            ...
            assert_eq!(java_api_return_type(&format!("custom.{owner}"), method, arity), None);
            assert_eq!(java_api_return_type(owner, &format!("{method}Other"), arity), None);
            assert_eq!(java_api_return_type(owner, method, 99), None);
        }
        assert_eq!(java_api_return_type("java.sql.Connection", "createStatement", 1), None);
