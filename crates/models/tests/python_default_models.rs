...
fn python_list_pop_propagates_receiver_to_return() {
    ...
    assert!(rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Receiver) && matches!(flow.to, Port::Return)));
    assert!(rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Receiver) && matches!(flow.to, Port::Receiver)));
}
...
fn python_dict_update_propagates_argument_into_receiver() {
    ...
    assert!(rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Arg(0)) && matches!(flow.to, Port::Receiver)));
    assert!(rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Receiver) && matches!(flow.to, Port::Receiver)));
...
fn python_comprehension_summaries_flow_into_return() {
    ...
    assert!(list_rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Arg(0)) && matches!(flow.to, Port::Return)));
    assert!(list_rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Arg(1)) && matches!(flow.to, Port::Return)));
    ...
    assert!(dict_rule
        .flows
        .iter()
        .any(|flow| matches!(flow.from, Port::Arg(2)) && matches!(flow.to, Port::Return)));
}
...
fn python_fastapi_path_params_is_a_source() {
    ...
    let call = python_call(
        ...
        Some("starlette.request.path_params"),
        "get",
        1,
    );
    assert!(rule.matcher.matches_call(&call));
...
fn python_current_app_config_get_is_modeled() {
    ...
    let source_call = python_call(
        "current_app.config.get",
        Some("flask.current_app.config"),
        "get",
        1,
    );
...
fn python_request_multidict_getlist_is_modeled() {
    ...
    let call = python_call(
        ...
        Some("starlette.datastructures.QueryParams"),
        "getlist",
        1,
    );
    assert!(rule.matcher.matches_call(&call));
...
fn python_django_queryset_distinct_is_modeled() {
    ...
    let call = python_call(
        "queryset.distinct",
        Some("django.db.models.QuerySet"),
        "distinct",
        0,
    );
    assert!(rule.matcher.matches_call(&call));
...
fn python_request_multidict_view_methods_are_modeled() {
    ...
    let call = python_call(
        ...
        Some("starlette.datastructures.Headers"),
        "items",
        0,
    );
    assert!(rule.matcher.matches_call(&call));
...
fn python_django_related_shape_chain_is_modeled() {
    ...
    let call = python_call(
        "author.books.select_related",
        Some("django.db.models.ManyRelatedManager"),
        "select_related",
        1,
    );
    assert!(rule.matcher.matches_call(&call));
...
fn python_fastapi_websocket_receive_summary_is_modeled() {
    ...
    let call = python_call(
        "ws.receive_text",
        Some("starlette.websockets.WebSocket"),
        "receive_text",
        0,
    );
    assert!(rule.matcher.matches_call(&call));
