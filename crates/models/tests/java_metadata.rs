...
fn jvm_metadata_preserves_original_prose_and_completes_all_locales() {
    ...
    let names = BTreeMap::from([
        ("sink".into(), "sql_injection".into()),
        ("unmapped".into(), "unknown".into()),
    ]);
    ...
    assert_eq!(
        entries[0].translations.zh_cn.as_ref().unwrap().message,
        "原始描述。\n\n请使用参数化查询。"
    );
    assert_eq!(
        entries[0].translations.zh_tw.as_ref().unwrap().message,
        "原始描述。\n\n請使用引數化查詢。"
    );
    ...
    assert_eq!(
        entries[1].translations.en.as_ref().unwrap().message,
        "fallback"
    );
    assert_eq!(
...
"#,
    ...
    let compilation =
        compile_legacy_jvm_rule_tree(&scratch.0, Language::Java, "legacy.java").unwrap();
    ...
    assert_eq!(
        compilation.metadata_report.unresolved_knowledge_ids,
        ["1234", "9999"]
    );
}
...
fn jvm_metadata_merge_is_idempotent_and_rejects_invalid_catalog_shapes() {
    ...
    assert!(catalog
        .ingest_yaml("BugInfos: {BugInfo: {id: invalid}}")
        .is_err());
    assert!(catalog
        .ingest_yaml("RuleSets: {RuleSet: [{Name: example, Maps: {Map: [{type: bug}]}}]}")
        .is_err());
}

#[test]
fn jvm_metadata_attaches_only_explicit_semantic_rulemap_aliases() {
    let mut catalog = LegacyJvmKnowledgeCatalog::default();
    catalog
        .ingest_yaml(
            r#"
RuleSets:
  RuleSet:
  - Name: command_injection_possible
    Maps:
      Map: [{type: bug, value: 'alias-command'}]
  - Name: similarly_named_but_unrelated
    Maps:
      Map: [{type: bug, value: 'must-not-attach'}]
"#,
        )
        .unwrap();
    let names = BTreeMap::from([("sink".into(), "command_injection".into())]);
    let mut entries = [metadata("sink")];
    catalog.enrich(&mut entries, &names);
    assert!(entries[0]
        .standards
        .contains(&"bug:alias-command".to_string()));
    assert!(!entries[0]
        .standards
        .iter()
        .any(|standard| standard.contains("must-not-attach")));
}
...
fn bundled_java_metadata_audit_matches_executable_sink_catalog() {
    ...
    assert_eq!(rules.sinks.len(), 4515);
    assert_eq!(rules.unused_return_sinks.len(), 1);
    assert_eq!(rules.metadata.len(), 4516);
    assert_eq!(
        report.mapped_sinks,
        rules
            .metadata
            ...
            .filter(|meta| meta
                .standards
                .iter()
                .any(|value| value.starts_with("LEGACY-MSG-")))
            .count()
    );
    assert_eq!(
        report.chinese_messages,
        rules
            .metadata
            .iter()
            .filter(|meta| meta
    ...
    let sql = rules
        .metadata
        .iter()
        .find(|meta| meta.id == "legacy.java.sink.bc4f4fcb-12de-41ab-81d6-6d9915c0e93e.0")
        .unwrap();
    ...
    assert_eq!(report.source_english_messages, 632);
    assert_eq!(report.source_traditional_chinese_messages, 0);
}
