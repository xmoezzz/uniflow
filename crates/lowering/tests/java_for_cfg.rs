...
fn for_continue_targets_update_and_backedge_targets_condition() {
    ...
    let hir = JavaParser::default().parse_file("Example.java", source).unwrap();
    let ir = lower_program(&hir);
    let function = ir.find_function_by_name("Example.run").unwrap();
    let call_block = |suffix: &str| function.blocks.iter().find(|block| block.insts.iter().any(|inst|
    ...
    assert!(matches!(ready.term, Terminator::Branch { then_bb, else_bb, .. }
...
fn for_header_phi_contains_initial_and_updated_values() {
    ...
    let assigned = function.blocks.iter().flat_map(|b| &b.insts).find_map(|inst| match &inst.kind {
        InstKind::Copy { dst, src } if *src == next_value => Some(*dst), _ => None,
    }).unwrap();
        ...
        for inst in &block.insts {
            if let InstKind::Call(call) = &inst.kind {
                if let Callee::Static(name) = &call.callee {
                    ...
                    if name.ends_with("replacement") { replacement = call.dst; }
                }
            }
        }
        match block.term {
