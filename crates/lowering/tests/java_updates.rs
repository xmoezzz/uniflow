...
fn lower(body: &str) -> Function {
    let source = format!("class Updates {{ int f(int i, int[] values, boolean flag) {{ {body} }} }}");
    let hir = JavaParser::default().parse_file("Updates.java", &source).unwrap();
...
fn steps(f: &Function) -> Vec<(ValueId, ValueId, bool)> {
    f.blocks.iter().flat_map(|b| &b.insts).filter_map(|i| match i.kind {
        InstKind::NumericStep { dst, src, increment } => Some((dst, src, increment)), _ => None,
    }).collect()
}

#[test]
fn java_updates_return_old_or_new_value_and_write_back_in_argument_order() {
    for (body, postfix, increment) in [("return i++;", true, true), ("return ++i;", false, true),
        ("return i--;", true, false), ("return --i;", false, false)] {
        let f = lower(body);
        let [(new, old, direction)] = steps(&f)[..] else { panic!("{f:#?}") };
    ...
    let [(first, old, true), (second, prior, true)] = steps(&f)[..] else { panic!("{f:#?}") };
...
fn java_updates_evaluate_field_receiver_and_array_subscript_once() {
    for body in ["return receiver().count++;", "return values[nextIndex()]++;", "return values[i++]++;"] {
        let f = lower(body);
        let instructions = f.blocks.iter().flat_map(|b| &b.insts).collect::<Vec<_>>();
        let calls = instructions.iter().filter(|i| matches!(i.kind, InstKind::Call(_))).count();
        assert_eq!(calls, usize::from(!body.contains("i++")), "{f:#?}");
        let (dst, old, _) = *steps(&f).last().unwrap();
        let load = instructions.iter().find(|i| matches!(i.kind,
            InstKind::LoadField { dst: d, .. } | InstKind::LoadIndex { dst: d, .. } if d == old)).unwrap();
        assert!(instructions.iter().any(|i| match (&load.kind, &i.kind) {
            (InstKind::LoadField { base: a, field: x, .. }, InstKind::StoreField { base: b, field: y, src }) => a == b && x == y && *src == dst,
            (InstKind::LoadIndex { base: a, index: x, .. }, InstKind::StoreIndex { base: b, index: y, src }) => a == b && x == y && *src == dst,
            _ => false,
        }), "{f:#?}");
    }
}
...
fn java_assignment_captures_location_before_rhs_update() {
    for body in ["values[i++] = i++; return i;", "return values[i++] = i++;"] {
        let f = lower(body);
        let [(first, old, true), (_, second_old, true)] = steps(&f)[..] else { panic!("{f:#?}") };
        assert_eq!(first, second_old);
        assert!(f.blocks.iter().flat_map(|b| &b.insts).any(|i| matches!(i.kind,
            InstKind::StoreIndex { index, src, .. } if index == old && src == first)), "{f:#?}");
    }
    for body in ["receiver().count = nextValue(); return i;", "return receiver().count = nextValue();"] {
        let f = lower(body);
        let calls = f.blocks.iter().flat_map(|b| &b.insts).filter_map(|i| match &i.kind {
            InstKind::Call(c) => match &c.callee { Callee::Static(n) => n.rsplit('.').next(), _ => None }, _ => None,
        }).collect::<Vec<_>>();
