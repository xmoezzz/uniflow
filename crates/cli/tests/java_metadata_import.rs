...
fn java_baseline_metadata_import_preserves_matchers_and_source_message_ids() {
    ...
    let scratch = Scratch(std::env::temp_dir().join(format!("uniflow-java-text-{}-{nonce}", std::process::id())));
    std::fs::create_dir(&scratch.0).unwrap();
    let knowledge = scratch.0.join("knowledge");
    std::fs::create_dir(&knowledge).unwrap();
    std::fs::write(knowledge.join("knowledge.yml"), r#"
BugInfos:
  BugInfo:
  - id: '00001'
    Categories:
...
"#;
    ...
    let result = Command::new(env!("CARGO_BIN_EXE_uniflow"))
        .args(["enrich-legacy-jvm-baseline", "--input"]).arg(&input)
        .arg("--knowledge").arg(&knowledge).arg("--output").arg(&output)
        .arg("--metadata-report-out").arg(&audit).output().unwrap();
    assert!(result.status.success(), "{}", String::from_utf8_lossy(&result.stderr));
    let enriched: serde_yaml::Value = serde_yaml::from_str(&std::fs::read_to_string(output).unwrap()).unwrap();
    let original: serde_yaml::Value = serde_yaml::from_str(original).unwrap();
    for field in ["id", "title", "languages", "severity", "confidence", "matcher", "message"] {
        assert_eq!(enriched["rules"][0][field], original["rules"][0][field]);
    }
    assert_eq!(enriched["rules"][0]["translations"]["zh-CN"]["message"].as_str(), Some("原始说明。\n\n原始建议。"));
    assert_eq!(enriched["rules"][0]["translations"]["zh-TW"]["message"].as_str(), Some("原始說明。\n\n原始建議。"));
    assert_eq!(enriched["rules"][0]["translations"]["en"]["message"].as_str(), Some("Existing reviewed message."));
...
fn java_ast_original_knowledge_text_executes_from_isolated_bundle() {
    let nonce = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos();
    let scratch = Scratch(std::env::temp_dir().join(format!("uniflow-java-knowledge-bundle-{}-{nonce}", std::process::id())));
    std::fs::create_dir(&scratch.0).unwrap();
    let fixture = scratch.0.join("Lock.java");
    std::fs::write(&fixture, "class A { Object lock = new Object(); void f() { synchronized(lock) { work(); } } }").unwrap();
    ...
    let output = Command::new(binary).current_dir(&scratch.0)
        .args(["check-baseline", "--language", "java", "--input"]).arg(fixture).output().unwrap();
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    let findings: Vec<serde_json::Value> = serde_json::from_slice(&output.stdout).unwrap();
    let finding = findings.iter().find(|finding| finding["rule_id"] == "LEGACY-JAVA-AST-sync-object-is-final").unwrap();
    assert_eq!(finding["translations"]["zh-CN"]["title"], "【强制】不要基于非final对象进行同步");
    assert!(finding["translations"]["zh-CN"]["message"].as_str().is_some_and(|message| !message.is_empty()));
    for locale in ["zh-CN", "en", "zh-TW"] {
        assert!(finding["translations"][locale]["title"].as_str().is_some_and(|title| !title.is_empty()));
        assert!(finding["translations"][locale]["message"].as_str().is_some_and(|message| !message.is_empty()));
    }
    assert_ne!(finding["translations"]["zh-CN"]["message"], finding["translations"]["zh-TW"]["message"]);
