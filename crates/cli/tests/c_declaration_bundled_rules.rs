...
fn c_declaration_rules_execute_from_isolated_binary() {
    ...
    let scratch = Scratch(std::env::temp_dir().join(format!("uniflow-c-declarations-{}-{unique}", std::process::id())));
    ...
    for suffix in ["void-fn-must-not-return-value", "non-void-fn-must-return", "non-void-fn-must-return-value",
        "func-decl-empty", "no-unnamed-argument", "no-unnamed-struct", "no-union-declaration-in-struct",
        "array-declaration-must-be-sized", "no-extern-declaration-in-function", "no-init-in-extern-declaration"] {
