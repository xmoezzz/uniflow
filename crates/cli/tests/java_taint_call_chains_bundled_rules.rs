...
fn scan(body: &str) -> Vec<serde_json::Value> {
    ...
    let scratch = Scratch(std::env::temp_dir().join(format!(
        "uniflow-java-call-chains-{}-{unique}",
        std::process::id()
    )));
...
fn check(body: &str, rule: &str) {
    ...
    let finding = findings
        .iter()
        .find(|finding| finding["sink_rule_id"] == rule)
        .unwrap();
    assert!(
    ...
    assert!(
        finding["standards"]
            ...
            .any(|value| value
                .as_str()
                .is_some_and(|value| value.starts_with("LEGACY-MSG-"))),
        "missing original mapping for {rule}"
    );
}
...
fn check_absent(body: &str, rule: &str) {
    ...
    assert!(
        findings
            .iter()
            .all(|finding| finding["sink_rule_id"] != rule),
...
fn java_script_engine_chain_executes_from_isolated_binary() {
    check(
        "engines.getEngineByName(\"js\").eval(request.getQueryString());",
...
fn java_path_constructor_executes_from_isolated_binary() {
    check(
        "new java.io.FileInputStream(request.getQueryString());",
...
fn java_redirect_executes_from_isolated_binary() {
    check(
        "servletResponse.sendRedirect(request.getQueryString());",
...
fn java_json_deserialization_executes_from_isolated_binary() {
    check(
        "mapper.readValue(request.getQueryString());",
...
fn java_yaml_deserialization_executes_from_isolated_binary() {
    check(
        "yaml.load(request.getQueryString());",
...
fn java_header_manipulation_executes_from_isolated_binary() {
    check(
        "url.openConnection().setRequestProperty(\"X-Test\", request.getQueryString());",
...
fn java_xstream_deserialization_executes_from_isolated_binary() {
    check(
        "xstream.fromXML(request.getQueryString());",
...
fn java_jndi_reference_injection_executes_from_isolated_binary() {
    check(
        "ldap.lookup(request.getQueryString(), contextMapper);",
