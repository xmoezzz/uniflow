...
fn c_expression_rules_execute_from_isolated_binary() {
    ...
    let scratch = Scratch(std::env::temp_dir().join(format!("uniflow-c-expressions-{}-{unique}", std::process::id())));
    ...
    for suffix in ["nullptr-zero", "no-assignment-outside-statement", "no-unary-in-expressions",
