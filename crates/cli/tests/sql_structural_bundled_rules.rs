...
#[test]
fn sql_structural_rules_execute_from_isolated_binary() {
    let unique = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos();
    let scratch = Scratch(std::env::temp_dir().join(format!("uniflow-sql-structural-{}-{unique}", std::process::id())));
    std::fs::create_dir(&scratch.0).unwrap();
    let fixture = scratch.0.join("SqlStructuralRules.sql");
    std::fs::write(&fixture, include_str!("fixtures/SqlStructuralRules.sql")).unwrap();
    ...
    for suffix in [
        "AddParenthesesInNestedExpression", "CollapsibleIfStatements", "ConcatenationWithNull",
        "DeclareSectionWithoutDeclarations", "DuplicateConditionIfElsif", "DuplicatedValueInIn",
        "EmptyBlock", "ExplicitInParameter", "FunctionWithOutParameter", "IdenticalExpression",
        ...
        "UselessParenthesis", "VariableInitializationWithFunctionCall", "VariableInitializationWithNull",
    ] {
