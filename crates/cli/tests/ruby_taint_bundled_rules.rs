...
fn assert_rules_execute_from_isolated_binary(cases: &[(&str, &str)]) {
    ...
    for (rule, source) in cases {
        ...
        let findings: Vec<serde_json::Value> = serde_json::from_slice(&output.stdout)
            .unwrap_or_else(|error| panic!("{expected}: {error}: {}", String::from_utf8_lossy(&output.stdout)));
        let actual = findings
            .iter()
            .filter_map(|finding| finding["sink_rule_id"].as_str())
            .collect::<HashSet<_>>();
        assert!(actual.contains(expected.as_str()), "missing {expected}: {actual:#?}");
        assert!(findings.iter().all(|finding| finding["analysis_complete"] == true));
    }
}
