...
fn c_preprocessor_rules_execute_from_isolated_binary() {
    let unique = SystemTime::now()
        ...
        .as_nanos();
    let scratch = Scratch(
        std::env::temp_dir().join(format!("uniflow-c-macros-{}-{unique}", std::process::id())),
    );
    ...
    let ids = findings
        ...
        .collect::<HashSet<_>>();
    for suffix in [
        "function-like-macros-must-have-braces",
        "no-redefine-keyword",
        "no-concat-in-macros",
        "no-concat-twice-in-macros",
        "no-macro-to-basic-type",
