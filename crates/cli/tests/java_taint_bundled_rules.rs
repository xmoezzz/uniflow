...
fn java_legacy_sql_taint_executes_from_isolated_binary() {
    ...
    let tainted_line = source
        .lines()
        .position(|line| line.contains("// tainted-sink"))
        .unwrap()
        + 1;
    assert!(
        sql.iter().all(|finding| finding["sink_location"]
            .as_str()
            .is_some_and(|location| location.contains(&format!(".java:{tainted_line}:")))),
        "{sql:#?}"
    );
    assert!(sql.iter().any(|finding| finding["source_rule_id"]
        == "legacy.java.source.ef3204b8-093b-4fa9-9b52-dc06e3e75533.0.web"));
    assert!(sql
        .iter()
        .all(|finding| ["zh-CN", "en", "zh-TW"].iter().all(|locale| {
            finding["translations"][locale]["title"]
                .as_str()
