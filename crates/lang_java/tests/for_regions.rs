...
fn java_for_preserves_declarations_condition_updates_and_body() {
    let source =
        "class ForRegions { void run() { for (int i=0, j=2; i<j; i++, --j) { continue; } } }";
    let program = JavaParser::default()
        .parse_file("ForRegions.java", source)
        .unwrap();
    ...
    assert!(init
        .stmts
        .iter()
        .all(|s| matches!(s, Stmt::Let { init: Some(_), .. })));
    assert!(matches!(
        cond,
        Some(Expr::Binary {
            op: BinaryOp::Lt,
    ...
    for (stmt, op) in update
        .stmts
        .iter()
        .zip([UnaryOp::PostIncrement, UnaryOp::PreDecrement])
    {
        assert!(
            matches!(stmt, Stmt::Expr { expr: Expr::Unary { op: actual, .. }, .. } if *actual == op)
        );
    }
...
fn java_project_merge_remaps_every_for_region() {
    let entries = vec![
        (
            ...
            "class First { void run() { for (int i=0; i<2; i++) work(i); } }".to_string(),
        ),
        (
            "Second.java".to_string(),
            "class Second { void run() { for (int j=0; j<2; j++) work(j); } }".to_string(),
        ),
    ];
    ...
    for module in &program.modules {
        for item in &module.items {
            ...
            for method in &class.methods {
                ...
                let Stmt::Let { symbol, .. } = init.stmts[0] else {
                    panic!("init")
                };
                assert!(
                    matches!(cond, Some(Expr::Binary { lhs, .. }) if matches!(lhs.as_ref(), Expr::VarRef { symbol: used, .. } if *used == symbol))
                );
                assert!(
                    matches!(&update.stmts[0], Stmt::Expr { expr: Expr::Unary { op: UnaryOp::PostIncrement, expr, .. }, .. }
...
fn java_lambda_captures_for_inputs_but_not_loop_declarations() {
    ...
    let Stmt::Let {
        init: Some(Expr::Lambda { captures, .. }),
        ..
    } = &class.methods[0].body.stmts[0]
    else {
        panic!("lambda")
