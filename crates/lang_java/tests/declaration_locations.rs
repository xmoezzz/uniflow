...
fn java_field_and_parameter_spans_reference_original_file_bytes() {
    ...
    let program = JavaParser::default().parse_file("Locations.java", source).unwrap();
    ...
    assert_eq!(&source[field_span.start_byte as usize..field_span.end_byte as usize], "String name;");
    ...
    for (param, declaration, column) in [
        ...
        assert_eq!(&source[param.span.start_byte as usize..param.span.end_byte as usize], declaration);
    }
}
...
"#;
    let program = JavaParser::default().parse_file("Locations.java", source).unwrap();
    let Item::Class(class) = &program.modules[0].items[0] else { panic!("class") };
    assert_eq!(class.methods.iter().map(|m| m.name.as_str()).collect::<Vec<_>>(),
    ...
    let return_type = program.types.iter().find(|t| Some(t.id) == first.return_type).unwrap();
